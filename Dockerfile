...
LABEL Description="Docker image for AWS ECR with Gradle" Vendor="Codehunters IO" Version="1.0"
...
RUN apk add --update --no-cache curl tar bash procps coreutils build-base python3 py3-pip zip docker
...
COPY awsecr-entrypoint.sh /awsecr-entrypoint.sh

RUN ln -fs /awsecr-entrypoint.sh /usr/local/bin/awsecr
RUN chmod +x /usr/local/bin/awsecr
